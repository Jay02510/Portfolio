...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, theme = 'dark' }) => {
  ...
  return (
    <article className="relative flex flex-col items-center">
      ...
      <div className={`w-full h-[350px] md:h-[700px] rounded-[1.5rem] md:rounded-[3rem] overflow-hidden border shadow-2xl relative ${theme === 'dark' ? 'border-white/10' : 'border-black/10'}`}>
          <img 
            src={project.imageUrl} 
            className={`w-full h-full object-cover ${project.imagePosition || 'object-center'} transition-all duration-1000 ${theme === 'dark' ? 'grayscale opacity-40 hover:grayscale-0' : 'opacity-90'}`} 
            alt={project.title}
          />
          <div className={`absolute inset-0 ${theme === 'dark' ? 'bg-gradient-to-t from-alpine-950 via-transparent' : 'bg-gradient-to-t from-alpine-50/80 via-transparent'}`}></div>
      ...
      {/* Detail Panel */}
      <div className={`mt-[-60px] md:mt-0 md:absolute ${isEven ? 'md:right-12' : 'md:left-12'} md:top-1/2 md:-translate-y-1/2 w-[90%] md:w-full md:max-w-xl glass-panel rounded-[1.5rem] md:rounded-[2.5rem] p-6 md:p-14 space-y-6 md:space-y-10 z-20`}>
          ...
          {project.betaCode && (
            <div className={`rounded-xl p-5 md:p-8 flex flex-col gap-3 md:gap-5 ${theme === 'dark' ? 'bg-white/5' : 'bg-black/[0.04]'}`}>
              <div className="flex justify-between items-center">
                 ...
                 {copied && <span className="text-[9px] font-bold text-accent-gold">COPIED</span>}
              </div>
              <div onClick={handleCopy} className="flex justify-between items-center cursor-pointer group">
                <code className={`text-base md:text-2xl font-mono tracking-widest px-3 md:px-6 py-2 md:py-4 rounded-lg border transition-all ${theme === 'dark' ? 'bg-white/5 border-white/5 group-hover:border-accent-gold/50 text-white' : 'bg-white border-black/5 text-alpine-950'}`}>
                  {project.betaCode}
                </code>
                <div className="text-[8px] md:text-[10px] font-bold uppercase tracking-[0.2em] opacity-40">Copy</div>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-4">
              ...
              >
                  Explore Demo
                  <ExternalLinkIcon className="w-4 h-4 opacity-40" />
              </a>
