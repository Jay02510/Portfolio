  ...
  return (
    <section className="py-24 px-6">
      <div className="max-w-3xl mx-auto">
        <div className={`glass-panel rounded-[2.5rem] p-10 md:p-16 relative overflow-hidden group transition-all duration-500 border ${theme === 'dark' ? 'border-white/10' : 'border-black/5'}`}>
          ...
          {!isSent ? (
            <div className="space-y-10 relative z-10">
              <div className="space-y-4">
                <div className={`text-[9px] font-bold tracking-[0.8em] uppercase ${theme === 'dark' ? 'text-accent-gold/40' : 'text-accent-clay/60'}`}>Honest Feedback</div>
                <h3 className="text-4xl font-light font-display tracking-tight text-gradient-white">
                  See a gap?
                </h3>
                <p className={`text-xs font-light leading-relaxed max-w-md transition-colors ${theme === 'dark' ? 'text-white/40' : 'text-alpine-950/60'}`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <textarea
                  ...
                  className={`w-full border rounded-2xl px-8 py-6 text-sm placeholder-current focus:outline-none transition-all font-light min-h-[140px] resize-none ${
                    theme === 'dark' 
                      ? 'bg-white/[0.03] border-white/10 text-white placeholder-white/20 focus:border-accent-gold/30' 
                      : 'bg-black/[0.04] border-black/10 text-alpine-950 placeholder-alpine-950/40 focus:border-accent-clay/30'
                  }`}
                  required
                />
                <button
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center space-y-6 animate-in fade-in zoom-in duration-500">
              ...
              <div className="space-y-2">
                <h4 className="text-2xl font-light font-display">Ready to send.</h4>
                <p className={`text-sm font-light ${theme === 'dark' ? 'text-white/30' : 'text-black/40'}`}>
